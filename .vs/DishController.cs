...
namespace LN_WEB.Controllers
{
    public class DishController : Controller
    {
        ...
        public ActionResult AddDish(HttpPostedFileBase ImageDish, DishEnt entidad)
        {
            ...
            string nombreArchivo = entidad.IdDish.ToString();  // Nombre del archivo basado en algún identificador único
            string ruta = Path.Combine(Server.MapPath(carpetaImagenes), nombreArchivo + extension);
        ...
        {
            string extension2 = Path.GetExtension(Path.GetFileName(ImageDish.FileName));
            string rutan = ConfigurationManager.AppSettings["pathDeleteImages"] + entidad.IdDish + extension2;
            System.IO.File.Delete(ConfigurationManager.AppSettings["pathDeleteImages"] + entidad.IdDish + extension2);
